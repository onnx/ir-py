...
def insert_nodes_in_value(
    ...
    if (graph := values[-1].graph) is not None:
        ...
        # Note nodes are inserted at the beginning when values are the graph inputs.
        target_node = values[0].producer()
        for v in values[1:]:
            if (new_target_node := v.producer()) is None:
                continue
            if target_node is None or graph.index(target_node) < graph.index(new_target_node):
                target_node = new_target_node
        if target_node is None:
            graph.insert_before(graph[0], new_nodes)
        else:
            graph.insert_after(target_node, new_nodes)

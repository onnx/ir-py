...
## Working with non-native NumPy dtypes: bfloat16, float8, float6, int4, int2

`onnx_ir.Tensor.numpy()` produces a NumPy array representation of the tensor's value. When the tensor has dtype `BFLOAT16`, `FLOAT8[...]`, `FLOAT6[...]`, `[U]INT4`, or `[U]INT2` which are not supported by NumPy, we use dtypes from the `ml_dtypes` package.

`float6`, `uint4`/`int4`, and `uint2`/`int2` are always unpacked; **`tobytes()` produces a packed representation** as expected.
